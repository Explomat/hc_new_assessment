...
class NumericInput extends Component{
    ...
    */
	...
	checkValidity()    {
		...
		const supportsValidation = !!this.refs.input.checkValidity;

        // noValidate
		const noValidate = !!(
            this.props.noValidate && this.props.noValidate.toString() !== 'false'
        );
     ...
     */
	_format(n)    {
		const _n = this._toNumber(n).toFixed(this.props.precision);
     ...
     */
	onMouseDown(dir = 'up' | 'down', callback)    {
	...
	render()    {
		...
		//console.log(omitObject(this.props, [ 'min' ]));

		/*let { mobile, style, step, min, max, precision, parse, format,
            value, type, defaultValue, onInvalid, onValid, ...rest } = this.props;*/

		let { mobile } = this.props;
		const { style } = this.props;
		const rest = omitObject(this.props,
			['mobile', 'style', 'step', 'min', 'max', 'precision', 'parse', 'format',
				'value', 'type', 'defaultValue', 'onInvalid', 'onValid']
        );

        // Build the styles
		for (const x in NumericInput.style) {
			if (Object.prototype.hasOwnProperty.call(NumericInput.style, x)) {
				css[x] = Object.assign(
                    {},
